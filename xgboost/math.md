...
We want to choose $f_t$ to minimize the regularized objective:

$$
\mathcal{L}^{(t)} = \sum_{i=1}^n l\left(y_i,\ \hat{y}_i^{(t)}\right) + \Omega(f_t) = \sum_{i=1}^n l\left(y_i,\ \hat{y}_i^{(t-1)} + f_t(x_i)\right) + \Omega(f_t)
$$

where $l$ is a differentiable convex loss function, $\Omega(f_t)$ is a regularization term penalizing tree complexity.
...
where $T$ is the number of leaves, $\gamma$ penalizes tree size, and $\lambda$ is an L2 penalty on leaf weights.
...
- $\lambda$ shrinks the leaf weight toward zero, preventing overfitting on leaves with few samples.
...
$$
w_j^* = -\frac{G_j}{H_j + \lambda} = -\frac{\sum_{i \in I_j} (\hat{y}_i^{(t-1)} - y_i)}{|I_j|+ \lambda} = \frac{\sum_{i \in I_j}(y_i - \hat{y}_i^{(t-1)})}{|I_j| + \lambda}
$$
...
$$
w_j^* = -\frac{G_j}{H_j + \lambda} = \frac{\sum_{i \in I_j} (y_i - p_i)}{\sum_{i \in I_j} p_i(1-p_i) + \lambda}
$$
